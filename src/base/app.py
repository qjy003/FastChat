...
from src.base.utils import return_type_must_belong
from src.base.utils import async_return_type_must_be_callable
        ...
        for attr, value in dct.items():
            ...
            if callable(value):
                ...
                if attr == 'inference' and is_over_write:
                    ...
                    dct[attr] = add_call_async_func_2_log(value)
...
class ChatApplication(ABC, metaclass=CombinedMeta):
    ...
    def __init__(self,
                 ...
                 port: int = PORT,
                 openai_api_key: str = AZURE_OPENAI_API_KEY if isinstance(AZURE_OPENAI_API_KEY, str) else random.choice(
        ...
        """
        self.port = int(os.environ.get("JD_ONLINE_PORT", port))
    ...
    async def think(self,
        ...
        async_task = [think_module.async_think(**kwargs.get(think_module_names[i], kwargs)) for i, think_module in
                      enumerate(think_modules)]
    ...
    def run(self):
        ...
        for module_name, module in all_modules.items():
            ...
            if llm_key not in llms:
                llms[llm_key] = MyChatOpenAI(model_name=getattr(module, 'model_name', self.gpt_model_name),
    ...
    async def query(self,
        ...
        tasks = [query_module['module'].async_query(
            query_content=get_dict_value(kwargs,
    ...
    async def _query(self, is_return_async_tasks: bool = True, **query_module_inputs) -> Union[Dict, Tuple[List,
        ...
        query_module_names = [model_name for model_name in query_module_inputs.keys() if model_name in
                              self.query_modules.keys()]
    ...
    # def disable_module(self, module_type: str, module_name: str = None):
    #     """
    #     Currently invalidates the module with the specified name and type
    #
    #     Args:
    #         module_type: the type of the module to be disabled
    #         module_name: the name of the module to be disabled
    #     """
    #     if module_type not in ['think', 'query', 'reply', 'evaluate']:
    #         raise ValueError("the module type must be 'think', 'query', 'reply' or 'evaluate'.")
    #     if module_name is None:
    #         if module_type == 'think':
    #             for module in self.think_modules:
    #                 setattr(module, "is_disable", True)
    #         if module_type == 'query':
    #             for module in self.query_modules:
    #                 setattr(module, 'is_disable', True)
    #         if module_type == 'reply':
    #             for module in self.reply_modules:
    #                 setattr(module, 'is_disable', True)
    #         if module_type == 'evaluate':
    #             for module in self.evaluate_modules:
    #                 setattr(module, 'is_disable', True)
    #     else:
    #         if module_type == 'think':
    #             if module_name not in self.think_modules:
    #                 raise ValueError(f"the module name {module_name} not exists in the think_modules.")
    #             setattr(self.think_modules.get(module_name), 'is_disable', True)
    #         if module_type == 'query':
    #             if module_name not in self.query_modules:
    #                 raise ValueError(f"the module name {module_name} not exists in the query_modules.")
    #             setattr(self.query_modules.get(module_name), 'is_disable', True)
    #         if module_type == 'reply':
    #             if module_name not in self.reply_modules:
    #                 raise ValueError(f"the module name {module_name} not exists in the reply_modules.")
    #             setattr(self.reply_modules.get(module_name), 'is_disable', True)
    #         if module_type == 'evaluate':
